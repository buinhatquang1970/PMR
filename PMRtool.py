...
def neutralize_df_for_excel(df):
    # Hỗ trợ cả Pandas bản cũ (applymap) và bản mới (map)
    try:
        if hasattr(df, 'map'):
        ...
        if hasattr(df_str, 'map'):
            return df_str.map(neutralize_excel_value)
        else:
            return df_str.applymap(neutralize_excel_value)
...
if is_admin_route:
    ...
    • Chọn Tỉnh/TP, độ cao (với mạng LAN).<br>
    • Đối với mạng WAN duplex: Cho phép chọn khoảng cách song công và tính toán ra kết quả là các cặp tần số sạch.<br>
    ...
    st.markdown("<h2 style='text-align: center; color: #4169E1;'>Ấn định tần số cho mạng nội bộ dùng riêng </h2>", unsafe_allow_html=True)
    ...
    with col_layout_left:
        ...
        with c_grp3:
            ...
            with col_conv:
                if st.button("Convert tọa độ", use_container_width=True): show_convert_popup()
            with col_map:
                if lat != 0 and lon != 0:
        ...
        c_mode, c_duplex, c_h, c_band, c_subband, c_bw = st.columns([1.2, 1.0, 0.7, 0.7, 1.8, 0.8], gap="small")
        ...
        with c_duplex:
            st.markdown("**K/c Song công**")
            is_duplex = (mode == "WAN_DUPLEX")
            duplex_spacing = st.selectbox("K/c Song công", [5, 10], format_func=lambda x: f"{x} MHz", disabled=not is_duplex, label_visibility="collapsed")
        ...
        with c_subband:
            ...
            if is_duplex:
                if duplex_spacing == 5:
                    if band == "VHF":
                        subbands = [
                            (142.0, 144.0, "142-144 / 147-149 MHz (B5mb)"),
                            (162.05, 165.0, "162.05-165 / 167.05-170 MHz (B5mb)")
                        ]
                    else: # UHF
                        subbands = [
                            (440.5, 441.0, "440.5-441 / 445.5-446 MHz (B5mb)"),
                            (441.2, 444.0, "441.2-444 / 446.2-449 MHz (B5mb)")
                        ]
                else: # 10MHz
                    if band == "VHF":
                        subbands = [] # Không có quy hoạch VHF cho k/c 10MHz
                    else: # UHF
                        subbands = [
                            (410.0, 415.0, "410-415 / 420-425 MHz (Trunking B10mb)"),
                            (415.5, 419.5, "415.5-419.5 / 425.5-429.5 MHz (B10mb)")
                        ]
                
                for s_f, e_f, label in subbands:
                    subband_map[label] = (s_f, e_f)
                    subband_labels.append(label)
                    
            else:
                current_alloc = ALLOC_VHF if band == "VHF" else ALLOC_UHF
            ...
            if not subband_labels:
                selected_subband_label = st.selectbox("Chọn dải con", ["-- Không có quy hoạch --"], disabled=True, label_visibility="collapsed")
        ...
        with c_qty:
            st.markdown("**Số cặp tần số**" if is_duplex else "**Số lượng tần số**")
    ...
    if btn_calc:
        ...
        if error_msg:
            ...
            mode_display = f"{mode} ({duplex_spacing}MHz)" if is_duplex else mode
            
            log_info(f"SESS: {st.session_state.session_id} |CALC_START|Pos:{lat:.6f},{lon:.6f}|Mode:{mode_display}|Band:{band}|H:{h_anten}|Subband:{selected_subband_label}|Prov:{prov_to_send}")
            with st.spinner('Đang tính toán...'):
                try:
                    tool = get_tool_instance(uploaded_files)
                    
                    # Truyền thêm tham số Duplex vào tool
                    user_input = {
                        "lat": lat, "lon": lon,
                        ...
                        "is_duplex": is_duplex,
                        "duplex_spacing": duplex_spacing if is_duplex else 0
                    }
                    results = tool.tinh_toan(user_input)
                    st.session_state.results = results
                    qty_label = "Số cặp xin" if is_duplex else "Số lượng xin"
                    st.session_state.input_snapshot = {
                        "THAM SỐ": ["Phiên bản App", "Kinh độ", "Vĩ độ", "Tỉnh / TP", "Độ cao Anten (m)", "Dải tần", "Phạm vi quét", "Băng thông", "Loại mạng", qty_label],
                        "GIÁ TRỊ": [APP_VERSION, f"{lon:.5f}", f"{lat:.5f}", prov_to_send if "LAN" in mode else "Toàn quốc (WAN)", h_anten, band, selected_subband_label, bw, mode_display, qty]
                    }
    ...
    if btn_scan_bad_freq:
        ...
        if not uploaded_files:
            ...
            mode_display = f"{mode} ({duplex_spacing}MHz)" if is_duplex else mode
            log_info(f"SESS: {st.session_state.session_id} | ACTION: SCAN_BAD_START | Pos: {lat:.6f},{lon:.6f} | Mode: {mode_display} | Band: {band} | Subband: {selected_subband_label}")
    ...
    if st.session_state.active_view == "AVAILABLE" and st.session_state.results is not None:
        ...
        if not results:
            ...
            # Khai báo tên cột linh hoạt dựa vào biến is_duplex
            freq_col_name = "Cặp TS Khả dụng (Tx/Rx)" if is_duplex else "Tần số Khả dụng (MHz)"
            
            # Gắn tên cột mới
            df_view.columns = ["STT", freq_col_name, "Hệ số Tái sử dụng (Điểm)", "Các GP sử dụng tần số này"]
            ...
            # Đổi linh hoạt nhãn Metric dựa vào biến is_duplex
            metric_label = "Cặp TS tốt nhất" if is_duplex else "Tần số tốt nhất"
            m2.metric(metric_label, f"{best_freq}")
            ...
            st.markdown(f"**Danh sách {qty} {'cặp ' if is_duplex else ''}tần số đề xuất tốt nhất:**")
            st.table(styler_top)
            ...
            if st.session_state.input_snapshot:
                ...
                df_export.rename(columns={
                    "STT": "STT",
                    "frequency": freq_col_name,  # <--- Sửa tại dòng này
